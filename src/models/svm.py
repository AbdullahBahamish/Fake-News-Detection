...
def run_svm():
    ...
    # Need to fit on train and transform on valid
    # Select columns by name implies passing the whole dataframe into the pipeline, 
    # but the pipeline expects specific columns. ColumnTransformer handles this if passed the DF.
    X_train_meta = metadata_pipeline.fit_transform(train_df)
    X_valid_meta = metadata_pipeline.transform(valid_df)
    
    # Combine
    print(" - Using metadata features only...")
    X_train = X_train_meta
    X_valid = X_valid_meta    
        
    # 4. Initialize SVM
    print("Initializing SVM (RBF kernel, Binary Classification)...")
    clf = SVC(kernel='rbf', C=1.0, gamma='scale', verbose=True)
